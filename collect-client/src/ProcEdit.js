...
export class ProcEdit extends LitElement {
  // use lightDOM
  createRenderRoot() {
    return this;
  }
  ...
  _closeForm() {
    ...
    this.dispatchEvent(
      new CustomEvent('close-procedure-edit', { bubbles: true, composed: true })
    );
  }
  ...
  async _clearFields() {
    ...
    this._procExecPlace = '';
    // @ts-ignore
    await document.getElementById('procedure-edit').reset();
    this._currentProcHour = '00';
    this._currentProcMinute = '00';
    this._procedureName = '';
    this._procTypeDescr = '';
    ...
    this._currentProcUsers = [];
    this._maxUsersCount = 5;
    // try to circunvent a race condition with the above procedure-edit reset
    setTimeout(() => {
  ...
  _saveForm(e) {
    e.preventDefault();
    if (document.getElementById('procedure-edit').reportValidity()) {
      ...
      <div class="modal ${classMap({ 'is-active': this.activate })}">
        ...
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Editar Procedimento</p>
            <button
              class="delete"
              aria-label="close"
